document.addEventListener("DOMContentLoaded", async function () {
  ...
  // ----------------- Initial Fetch -----------------
  await fetchAllCars(); // Fetch and display all cars when the page loads
  ...
  viewMakeResultsBtn.addEventListener("click", function () {
    const selectedMakeEl = document.querySelector("input[name='make']:checked");
    if (selectedMakeEl) {
      console.log("Filtering cars by make:", selectedMakeEl.value);
    }
    resetFilters();
    closeAllModals();
  });
  ...
  viewTypeResultsBtn.addEventListener("click", function () {
    ...
    if (selectedTypeEl) {
      console.log("Filtering cars by type:", selectedTypeEl.value);
    }
    resetFilters();
    closeAllModals();
  });
  ...
  viewYearResultsBtn.addEventListener("click", function () {
    const selectedYearEl = document.querySelector("input[name='year']:checked");
    if (selectedYearEl) {
      console.log("Filtering by Year:", selectedYearEl.value);
    }
    document.getElementById("year-modal").style.display = "none"; // Close modal
  });
  ...
  // ----------------- Apply Filters (Date Availability) -----------------
  document.getElementById("apply-filters").addEventListener("click", async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/booking/cars/available?startDate=${encodeURIComponent(startDate)}&endDate=${encodeURIComponent(endDate)}`,
        {
          method: 'GET',
          headers: {
      ...
      if (response.ok) {
        ...
        if (availableCars.length === 0) {
          carListContainer.innerHTML = `<p style="text-align: center; font-size: 16px;">No cars available for the selected dates.</p>`;
        } else {
        ...
        alert(availableCars.message || "Failed to fetch available cars.");
      }
    } catch (error) {
  ...
  if (queryStartDate && queryEndDate) {
    document.getElementById("start-date").value = queryStartDate;
    document.getElementById("end-date").value = queryEndDate;
    try {
      const response = await fetch(
        `http://localhost:8080/api/booking/cars/available?startDate=${encodeURIComponent(queryStartDate)}&endDate=${encodeURIComponent(queryEndDate)}`,
        {
          method: 'GET',
      ...
      const availableCars = await response.json();
      if (response.ok) {
        console.log("Available cars from URL params:", availableCars);
        carListContainer.innerHTML = "";
        if (availableCars.length === 0) {
          carListContainer.innerHTML = `<p style="text-align: center; font-size: 16px;">No cars available for the selected dates.</p>`;
        } else {
        ...
        console.error("Error from availability API:", availableCars.message);
      }
    } catch (error) {
      console.error("Error fetching available cars on page load:", error);
    }
  }
  ...
  async function fetchAllCars() {
    try {
      ...
      cars.forEach(car => {
        ...
        carItem.classList.add("car-item");
        carItem.innerHTML = `
          <img src="${car.image_url || '../assets/images/default-car.jpg'}" alt="${car.make} ${car.model}" class="car-image">
          <div class="car-info">
            <h3>${car.make} ${car.model}, ${car.year}</h3>
          </div>
          <div class="car-price">$${car.price_per_day} per day</div>
  ...
  viewMakeResultsBtn.addEventListener("click", () => {
    const selectedMakeEl = document.querySelector("input[name='make']:checked");
    selectedFilters.make = selectedMakeEl ? selectedMakeEl.value : null;
    fetchAndDisplayCars();
  });

  viewTypeResultsBtn.addEventListener("click", () => {
    const selectedTypeEl = document.querySelector("input[name='type']:checked");
    selectedFilters.type = selectedTypeEl ? selectedTypeEl.value : null;
    fetchAndDisplayCars();
  });

  viewYearResultsBtn.addEventListener("click", () => {
    const selectedYearEl = document.querySelector("input[name='year']:checked");
    selectedFilters.year = selectedYearEl ? selectedYearEl.value : null;
    fetchAndDisplayCars();
  });

  viewPriceResultsBtn.addEventListener("click", () => {
    const selectedPriceEl = document.querySelector("input[name='price-sort']:checked");
    selectedFilters.price = selectedPriceEl ? selectedPriceEl.value : null;
    fetchAndDisplayCars();
  });
